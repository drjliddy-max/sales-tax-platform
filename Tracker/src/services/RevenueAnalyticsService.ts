...
export class RevenueAnalyticsService {
  ...
  /**
   * Calculate Monthly Recurring Revenue with optional filters
   */
  static async calculateMRR(
    startDate?: Date,
    endDate?: Date,
    tierFilter?: mongoose.Types.ObjectId
  ): Promise<MRRResult> {
    try {
      ...
      const result = await ClientSubscription.aggregate([
  ...
  /**
   * Get revenue breakdown by stream for a date range
   */
  static async getRevenueByStream(
    startDate: Date,
    endDate: Date
  ): Promise<RevenueStreamAnalytics[]> {
    try {
  ...
  /**
   * Get revenue breakdown by client tier
   */
  static async getRevenueByTier(
    startDate: Date,
    endDate: Date
  ): Promise<TierAnalytics[]> {
    try {
      const result = await ClientTier.aggregate([
        {
          $match: { isActive: true }
        },
        {
  ...
  /**
   * Get basic revenue summary for a date range
   */
   ...
   */
  static async getRevenueGrowthRate(
    currentPeriodStart: Date,
    currentPeriodEnd: Date,
    previousPeriodStart: Date,
    previousPeriodEnd: Date
  ): Promise<RevenueGrowthRate> {
    try {
  ...
  /**
   * Get customer lifetime value by tier
   */
  static async getCustomerLifetimeValueByTier(): Promise<any[]> {
    try {
      const result = await ClientTier.aggregate([
        {
          $lookup: {
   ...
   * Get churn analysis
   */
  static async getChurnAnalysis(startDate: Date, endDate: Date): Promise<any> {
    try {
