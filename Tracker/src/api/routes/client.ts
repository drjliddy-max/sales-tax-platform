...
import { protectClientRoute } from '@/middleware/auth';
...
router.get('/dashboard/overview', async (req, res) => {
  try {
    ...
    const currentMonthMetrics = await prisma.transaction.aggregate({
      where: {
    ...
    // Upcoming tax deadlines (mock data - you'd integrate with real tax calendar)
...
router.get('/compliance/overview', async (req, res) => {
  try {
    ...
    // Calculate current month tax liability
    const currentMonth = new Date();
    currentMonth.setDate(1);
    currentMonth.setHours(0, 0, 0, 0);

    const currentMonthTax = await prisma.transaction.aggregate({
      where: {
        businessId: { in: businessIds },
        transactionDate: { gte: currentMonth },
        status: 'COMPLETED'
    ...
    // Mock compliance score calculation (in real implementation, this would be more complex)
    const complianceScore = 95; // Based on filing history, accuracy, etc.
    ...
    res.json({
      ...
      taxLiability: {
        ...
        overdue: 0 // Would calculate from past due filings
      },
      filingStatus,
      upcomingDeadlines
    });
...
router.get('/analytics/insights', async (req, res) => {
  try {
    ...
    // Sales trends
    const salesTrends = await prisma.transaction.groupBy({
      by: ['transactionDate'],
      where: {
        businessId: { in: businessIds },
    ...
    const categoryPerformance = await prisma.transaction.groupBy({
      by: ['productCategory'],
      where: {
        businessId: { in: businessIds },
        transactionDate: { gte: dateFilter },
    ...
    const locationPerformance = await prisma.transaction.groupBy({
      by: ['saleLocation'],
      where: {
        businessId: { in: businessIds },
        transactionDate: { gte: dateFilter },
