...
class POSIntegrationFactory {
    ...
    static generateAdapterTest(template) {
    ...
    static createIntegrationChecklist(posType) {
        return {
    ...
    static validateIntegration(posType, adapter, checklist) {
        const missingItems = [];
    ...
    static async registerAdapter(posType, adapter, template) {
        try {
            const fingerprint = {
            ...
            detection_1.POSDetector.getConfigurationTemplate(posType);
...
describe('{{ADAPTER_CLASS}}', () => {
  ...
  describe('transformRawData', () => {
    ...
    static getAdapterClassName(posType) {
        return posType
            .split('_')
            .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
            .join('') + 'Adapter';
    }
    static generateTestCredentials(template) {
        const credentials = { type: template.authMethod };
...
exports.POSIntegrationFactory = POSIntegrationFactory;
POSIntegrationFactory.ADAPTER_TEMPLATE_PATH = (0, path_1.join)(__dirname, 'templates', 'adapter-template.ts');
POSIntegrationFactory.TEST_TEMPLATE_PATH = (0, path_1.join)(__dirname, 'templates', 'adapter-test-template.ts');
class IntegrationGuide {
    static getDevelopmentSteps(posType) {
        return [
...
exports.IntegrationGuide = IntegrationGuide;
//# sourceMappingURL=integration-framework.js.map