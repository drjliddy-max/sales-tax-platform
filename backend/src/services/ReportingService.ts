...
export class ReportingService {
    ...
    try {
      ...
      for (const scheduledReport of dueReports) {
        ...
        if (result.success) {
          ...
          console.error(`Failed to generate scheduled report ${scheduledReport.name}:`, result.error);
        }
      }
    } catch (error) {
  ...
  private static async deliverReport(scheduledReport: any, reportId: string): Promise<void> {
    console.log(`Delivering report ${reportId} via ${scheduledReport.deliveryMethod} to:`, scheduledReport.recipients);
