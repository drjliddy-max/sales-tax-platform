...
export class RevenueService {
  ...
  /**
   * Initialize sample revenue data for development
   */
  static async initializeSampleData(): Promise<void> {
    try {
      ...
      const revenueStreams = await RevenueStream.insertMany([
        {
      ...
      // Create client tiers
      const clientTiers = await ClientTier.insertMany([
        {
          name: 'Starter',
          monthlyPrice: 29.00,
          annualPrice: 290.00,
      ...
      const createdSubscriptions = await ClientSubscription.insertMany(subscriptions);
      ...
      await RevenueTransaction.insertMany(revenueTransactions);
  ...
  /**
   * Get revenue transactions for admin dashboard
   */
  static async getAdminRevenueTransactions(): Promise<any[]> {
    try {
      ...
      return transactions.map(transaction => ({
        ...
        customer: (transaction.clientId as any).name,
        items: [transaction.description || (transaction.revenueStreamId as any).name],
        transactionType: (transaction.revenueStreamId as any).category,
  ...
  static async getRevenueMetrics(): Promise<{
    ...
    try {
      const [mrrResult, clientCount, subscriptionCount] = await Promise.all([
        ...
        ClientSubscription.countDocuments({ status: 'active' })
      ]);
